...
<template>
  ...
  >
    ...
    <div class="grid-x" v-if="cafe.company.cafes_count > 1">
      <div class="large-12 medium-12 small-12 cell">
        <span class="location-number">
          ...
          {{ cafe.company.cafes_count }} other locations
        </span>
      </div>
    </div>
    <div class="grid-x">
      <div class="large-12 medium-12 small-12 cell">
        ...
        <div class="location-type restaurant" v-if="cafe.company.restaurant == 1">
          <img src="/img/restaurant-logo.svg"> Restaurant
        </div>
        <div class="location-type cafe" v-if="cafe.company.restaurant == 0">
    ...
    <div
      class="grid-x"
      v-if="cafe.company.instagram_url != null || cafe.company.facebook_url != null || cafe.company.twitter_url != null"
    >
      <div class="large-12 medium-12 small-12 cell">
        <a
          v-bind:href="cafe.company.instagram_url"
          v-if="cafe.company.instagram_url != null"
          target="_blank"
        >
          <img src="/img/instagram-logo.svg" class="social-icon">
        </a>
        <a
          v-bind:href="cafe.company.facebook_url"
          v-if="cafe.company.facebook_url != null"
          target="_blank"
        >
          <img src="/img/facebook-logo.svg" class="social-icon">
        </a>
        <a
          v-bind:href="cafe.company.twitter_url"
          v-if="cafe.company.twitter_url != null"
          target="_blank"
        >
    ...
    <div class="grid-x">
      <div class="large-12 medium-12 small-12 cell">
        ...
        <a
          class="cafe-website"
          target="_blank"
          v-bind:href="cafe.company.website"
        >{{ cafe.company.website }}</a>
        <br>
        <router-link
          :to="{ name: 'editcafe', params: { slug: cafe.slug } }"
          v-show="userLoadStatus == 2 && user != ''"
          class="suggest-cafe-edit"
        >Suggest an edit</router-link>
        <a
          class="suggest-cafe-edit"
          v-if="userLoadStatus == 2 && user == ''"
          v-on:click="loginToEdit()"
        >Sign in to make an edit</a>
      </div>
    </div>
  </div>
</template>
