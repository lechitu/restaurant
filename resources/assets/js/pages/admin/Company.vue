  ...
  <div id="admin-company">
    ...
    <div class="grid-container">
      <div class="grid-x admin-tabs">
        <div
          ...
          v-bind:class="{'active-tab': tab == 'information'}"
        >Information</div>
        <div
          class="tab"
          v-on:click="tab = 'cafes'"
          v-bind:class="{'active-tab': tab == 'cafes'}"
        >Cafes</div>
        <div
          class="tab"
          v-on:click="tab = 'history'"
          v-bind:class="{'active-tab': tab == 'history'}"
        >History</div>
      </div>
    </div>
      ...
      <div class="grid-x">
        <div class="large-8 medium-12 cell">
          ...
          <div
            ...
            v-bind:class="{ 'active': type == 'restaurant' }"
            v-on:click="setCompanyType('restaurant')"
          >Restaurant</div>
          <div
            class="location-type cafe"
            v-bind:class="{ 'active': type == 'cafe' }"
            v-on:click="setCompanyType('cafe')"
          >Cafe</div>
        </div>
      </div>
      <!-- <div class="grid-x grid-padding-x" v-show="type == 'restaurant'">
        <div class="large-8 medium-9 small-12 cell centered">
          <label>Does the restaurant offer a subscription service?</label>
        </div>
      </div>
        ...
        <div class="large-8 medium-12 cell">
          ...
          <div class="owner" v-for="(owner, key) in owners">
            <router-link
              ...
              :to="{name: 'admin-user', params: { 'id': owner.id } }"
          ...
          <div class="user-selection-container" v-if="user.permission > 1">
            ...
            <div
              class="user-autocomplete-container"
              v-show="newOwner.length > 0 && showAutocomplete"
            >
              <div
                class="user-autocomplete"
                v-for="user in newOwnerResults"
    ...
    <div class="grid-container" v-show="tab == 'cafes'">
      <div class="grid-x cafes-header">
        <div class="large-3 medium-3 cell">Location Name</div>
        <div class="large-6 medium-6 cell">Address</div>
        <div class="large-3 medium-3 cell"></div>
      </div>
      <cafe v-for="cafe in company.cafes" :cafe="cafe" :key="cafe.id"></cafe>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    syncCompanyToModel() {
      ...
      this.type = this.company.restaurant == 1 ? "restaurant" : "cafe";
    ...
    searchUsers: _.debounce(function(e) {
      ...
      if (this.newOwner.length > 1) {
        ...
        /*
          ...
          */
        axios
          .get(ROAST_CONFIG.API_URL + "/admin/users", {
            params: {
