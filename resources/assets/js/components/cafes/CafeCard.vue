...
<template>
  <div class="large-3 medium-4 small-6 cell cafe-card-container" v-show="show">
    <router-link
      :to="{ name: 'cafe', params: { slug: cafe.slug} }"
      v-on:click.native="panToLocation( cafe )"
    >
      <div class="cafe-card">
        ...
        <div class="meta-data">
          <span class="liked-meta">
            <img v-bind:src="cafe.user_like_count > 0 ? '/img/liked.svg' : '/img/unliked.svg'">
            {{ cafe.likes_count }}
          </span>
        </div>
      </div>
  ...
  methods: {
    ...
    processFilters() {
      ...
      if (
        ...
        if (
          this.textSearch != "" &&
          this.processCafeTextFilter(this.cafe, this.textSearch)
        ) {
          textPassed = true;
        ...
        if (
          this.brewMethodsFilter.length != 0 &&
          this.processCafeBrewMethodsFilter(this.cafe, this.brewMethodsFilter)
        ) {
        ...
        if (
          this.hasSubscription &&
          this.processCafeSubscriptionFilter(this.cafe)
        ) {
        ...
        if (
          this.cityFilter != "" &&
          this.processCafeInCityFilter(this.cafe, this.cityFilter)
        ) {
          cityPassed = true;
