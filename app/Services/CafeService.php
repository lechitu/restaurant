  ...
  public static function addCafe( $data, $addedBy ){
    ...
    */
    if( $companyID != '' ){
      ...
      $company->restaurant			  = $data['company_type'] == 'restaurant' ? 1 : 0;
    ...
    */
    ...
    if( $lat == 0 && $lng == 0 ){
      $coordinates = GoogleMaps::geocodeAddress( $address, $city, $zip );
      $lat = $coordinates['lat'];
      $lng = $coordinates['lng'];
    }
    ...
    */
    ...
    $cafe->slug 						= SlugService::createSlug(Cafe::class, 'slug', $company->name.' '.$locationName.' '.$address.' '.$city);
      ...
      */
      if( isset( $data['company_id'] ) ){
        ...
        */
        ...
        /*
          If the data has a company type, add the type but default to not a restaurant.
        */
        if( isset( $data['company_type'] ) ){
          $company->restaurant	= $data['company_type'] == 'restaurant' ? 1 : 0;
        }else{
          $company->restaurant = 0;
        }
      ...
      */
      if( ( isset( $data['address'] ) && $data['address'] != $cafe->address )
          ...
          || ( isset( $data['zip'] ) && $data['zip'] != $cafe->zip ) ){
            $coordinates = GoogleMaps::geocodeAddress( $data['address'], $data['city'], $data['zip'] );

            $slug = SlugService::createSlug(Cafe::class, 'slug', $company->name.' '.$locationName.' '.$address.' '.$city);
